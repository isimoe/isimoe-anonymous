...
def parse_args():
    ...
    parser.add_argument(
        "--amss_enabled", type=str2bool, default=False, help="Enable M2-AMSS adaptive masking"
    )
    parser.add_argument(
        "--amss_tau", type=float, default=1.0, help="Temperature for AMSS softmax scaling"
    )
    parser.add_argument(
        "--amss_momentum", type=float, default=0.9, help="Momentum for smoothing modal significance estimates"
    )
    parser.add_argument(
        "--scale_factor", type=float, default=1.0, help="Masking scale factor"
    )
    parser.add_argument(
        "--topk_ratio", type=float, default=0.2, help="Top-k ratio for correlated part extraction"
    )
    parser.add_argument(
        "--amss_aux_weight", type=float, default=0.0, help="Weight for auxiliary per-expert supervision (L_aux)"
    )
    parser.add_argument(
        "--amss_orth_weight", type=float, default=0.0, help="Weight for orthogonality regularizer between experts (L_orth)"
    )
    parser.add_argument(
        "--ldiv_level",
        type=str,
        ...
        help="Representation used by L_div/L_orth: intermediate features or final predictions",
    )
    parser.add_argument(
        "--use_interaction", type=str2bool, default=True, help="Whether to use Cross-Attention interaction"
    )
    parser.add_argument(
        "--enable_r_path",
        type=str2bool,
        default=False,
        help="Whether to enable the optional redundancy expert path",
    )
    parser.add_argument(
        "--track_mir_curve",
        type=str2bool,
        default=False,
        help="Record batch-level MIR values during training and plot modality trajectories",
    )
    parser.add_argument(
        "--mir_log_dir",
        type=str,
        default="./outputs/mir_curves",
        help="Directory for MIR trajectory CSV and figures",
    )
